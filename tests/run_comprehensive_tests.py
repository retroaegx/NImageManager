...
class ComprehensiveIntegrationTests(unittest.TestCase):
    ...
    def test_ui_regression_contracts(self):
        with Runtime() as rt:
            admin_js = (rt.temp_root / "server" / "web" / "admin.js").read_text(encoding="utf-8")
            settings_js = (rt.temp_root / "server" / "web" / "settings.js").read_text(encoding="utf-8")
            app_js = (rt.temp_root / "server" / "web" / "app.js").read_text(encoding="utf-8")
            index_html = (rt.temp_root / "server" / "web" / "index.html").read_text(encoding="utf-8")

            self.assertIn("作品、ブックマーク、作者登録、共有ブックマーク登録、関連データも削除します", admin_js)
            self.assertGreaterEqual(admin_js.count("confirm("), 2)
            self.assertIn("confirmMyAccountDelete", settings_js)
            self.assertIn("本当に削除しますか？", settings_js)
            self.assertIn("作者登録", app_js)
            self.assertIn("共有ブックマーク登録", app_js)
